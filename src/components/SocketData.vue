    ...
    <div class="row justify-content-around">
      <div class="col-3">
        ...
        <div class="card" style="width: 18rem;">
          <div v-if="socketId" class="card-header">
            <h3>
          ...
          <ul v-if="welcomeData" class="list-group list-group-flush">
            <li class="list-group-item">ID Conexion: {{ socketId }}</li>
            <li class="list-group-item">Tienda: {{ welcomeData.location_name }}</li>
            <li class="list-group-item">Capacidad Maxima: <strong>{{ welcomeData.max_capacity }} personas</strong></li>
            <li class="list-group-item">Localidad: {{ welcomeData.location_address }}</li>
          </ul>
        </div>
      </div>

      <div class="col-3">
        <div v-if="heartbeatData" class="card" style="width: 18rem;">
          ...
          <ul class="list-group list-group-flush">
            <li class="list-group-item">Aforo actual: {{ heartbeatData.current_aforo }}</li>
            <li class="list-group-item">
              <p v-if="heartbeatData.monitorView === 'IN'" class="text-success">Pueden seguir ingresando al local.
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
              ...
              <p v-else-if="heartbeatData.monitorView === 'WARNING'" class="text-warning">Local con espacio reducido.
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
              ...
              <p v-else-if="heartbeatData.monitorView === 'STOP'" class="text-danger">Local Lleno.
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            ...
            <li class="list-group-item">Personas Ingresadas: {{ heartbeatData.counting_in }}</li>
            <li class="list-group-item">Personas Salientes: {{ heartbeatData.counting_out }}</li>
            <li class="list-group-item">Fecha de actualizacion: {{ heartbeatData.datetime_event }} HUSO HORARIO UTC+0
            </li>
          </ul>
        </div>
        <div v-else>
          <h5>Nuestros datos podrian tardar hasta 30seg en actualizarse en cada solicitud...</h5>
...
export default {
  setup() {
    ...
    const socketId = computed(() => socketStore.socketId);
    const welcomeData = computed(() => socketStore.welcomeData);
    const heartbeatData = computed(() => socketStore.heartbeatData);
